...
import { useAddUser, useCheckAvailibility, useCreateSession, useGetAllUsers } from "../services";
...
const useCreateSessionController = ({ onClose }: CreateSessionControllerProps) => {
    ...
    const createNewClient = useAddUser();
    ...
    const handleAddPatient = async (patientData: IAddUser) => {
        try {
            ...
            setForm(prev => ({ ...prev, patient: newPatient.user._id, isNewClient: true }));
        } catch (error) {
